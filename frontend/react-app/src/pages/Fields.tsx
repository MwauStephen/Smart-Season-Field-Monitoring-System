...
export default function FieldsPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      toast.error("Failed to sync field data.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateField = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newField.assigned_agent_id) {
      toast.error("Please assign an agent to this field.");
      return;
    }
    try {
    ...
    return (
      <div className="h-full flex items-center justify-center p-20">
         <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-muted-foreground font-medium animate-pulse">Loading Fields...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
            ...
            <DialogContent className="sm:max-w-[425px]">
              <form onSubmit={handleCreateField}>
                ...
                <DialogFooter>
                  <Button type="submit" className="bg-brand-green text-white">Create Field</Button>
                </DialogFooter>
              </form>
            </DialogContent>
          </Dialog>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center">
        ...
        <Button variant="outline" className="bg-white border-none shadow-sm h-11">
                ...
                <div className="flex justify-between items-start">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="font-bold text-lg text-brand-dark dark:text-white">{field.name}</h3>
                    </div>
                ...
                {isAdmin && (
                  <div className="flex items-center gap-2 pt-2 border-t border-border/50">
                    ...
                    <span className="text-xs font-medium text-muted-foreground">
                      Assigned to: <span className="text-brand-dark dark:text-white font-bold">{agents.find(a => a.id === field.assigned_agent_id)?.name || "Agent"}</span>
                    </span>
                  </div>
                )}
              </div>
      ...
      <Dialog open={isUpdateOpen} onOpenChange={setIsUpdateOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <form onSubmit={handleLogUpdate}>
            ...
            <DialogFooter>
              <Button type="submit" className="bg-brand-green text-white">Save Update</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
